...
public class MiniMax
{
	...
	public delegate void AIMoveGeneratedCallBack(MoveData move);
	public Thread GetMove()
	{
		...
		// StartCoroutine(CalculateMinMax(maxDepth, int.MinValue, int.MaxValue, true));
		Thread thread1 = new Thread(() => CalculateMinMax(maxDepth, int.MinValue, int.MaxValue, true)) {IsBackground= true};
		thread1.Start();
		//thread1.Join();
	...
	// IEnumerator CalculateMinMax(int depth, int alpha, int beta, bool max)
	// {
	// 	GetBoardState();
	...
	// 			DoFakeMove(move.firstPosition, move.secondPosition);
	// 			CoroutineWithData cd = new CoroutineWithData(this, CalculateMinMax(depth - 1, alpha, beta, false) );
	...
	// 			DoFakeMove(move.firstPosition, move.secondPosition);
	// 			CoroutineWithData cd = new CoroutineWithData(this, CalculateMinMax(depth - 1, alpha, beta, true) );
 	// 			yield return cd.coroutine;
	// 			score = (int)cd.result;
